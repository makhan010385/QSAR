...
def smiles_to_simple_descriptors(smiles):
    """Generate deterministic, lightweight descriptors from a SMILES string.

    Feature names are normalized to lowercase so they remain consistent
    after standardize_columns() is applied on Streamlit Cloud.
    """
    if smiles is None:
        s = ""
    else:
                ...
                for pic_idx, row in df_pic50.iterrows():
                    ...
                    if found_idx is not None:
                        ...
                        for key, value in generated.items():
                            ...
                            if key_norm in descriptor_row.index:
                                descriptor_row[key_norm] = value
